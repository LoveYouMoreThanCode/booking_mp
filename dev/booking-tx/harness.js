/* ══════════════════════════════════════════════════════════════
   harness.js —— 在本地把三个「写路径」云函数跑起来

   覆盖 createBooking / updateBooking / clearAll。这三个是同一套东西的
   三面：都只在真云上跑过才作数，而它们各自都有一类【在真机上看起来
   只是没反应】的错：
...
function makeDb() {
  ...
  function docRef(c, id) {
    return {
      ...
      set: function (a) { coll(c)[id] = clone(a.data); tick(c, id); return Promise.resolve({}); },
      update: function (a) {
        var d = coll(c)[id];
...
print('══ 写路径云函数本地台架（createBooking / updateBooking / clearAll）══');
